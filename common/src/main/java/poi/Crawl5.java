...
//        List<String> titleList = Arrays.asList("home","resident","same_resident","work");
        String path = "D:\\work\\demo\\common\\src\\main\\resources\\qu\\" + "客流指数查询分析1.xlsx";
        ...
        for (int i=0;i<mapList.size();i++) {
            ...
            try{
                ...
                while (iterator.hasNext()){
                    ...
                    values[i][number] = value.getString("day_avg");
                    number = number + 1;

                    JSONObject hourAvg = value.getJSONObject("hour_avg");
                    for(int j=0;j<24;j++){
                        values[i][number] = hourAvg.getString(String.valueOf(j));
                        number = number + 1;
                    }
                }
            }catch (Exception e){
