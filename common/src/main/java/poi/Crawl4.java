...
//
    ...
    String url = "https://apis.map.qq.com/bigdata/popinsight/v1/residentuserprofile";
        ...
        String path = "D:\\work\\demo\\common\\src\\main\\resources\\qu\\" + "常驻人口画像分析.xlsx";
        String[][] values = new String[mapList.size()+1][1000];
        for (int i=0;i<mapList.size();i++) {
            int number = 1;
            Map<String, String> map = mapList.get(i);
            String name = map.get("name");
            values[i][0] = name;
            String params = "{ \"boundary_type\": \"circle\", \"center\": \"%s\", \"range\": \"500\", \"adcode\": \"%s\", \"min_area\": \"10000\", \"month\": \"201911\", \"people_type\": \"4\", \"label\":\"101010,101012,101015,101110,1012,1013,1112,1124,1120,1121,1125,1110,1117,1116,1119,1310,1311,1312,1401,1501,1610,1612\", \"key\": \"UJMBZ-T35WG-UEDQL-IGGWO-7V7TV-OGB4A\" }";
            ...
            try{
                ...
                while (iterator.hasNext()){
                    ...
                    while (iteratorValue.hasNext()) {
                        ...
                        JSONObject value1 = (JSONObject) entryValue.getValue();
                        values[i][number] = value1.getString("TGI");
                        number = number +1;
                        values[i][number] = value1.getString("percent");
                        number = number +1;
                    }
                }
            }catch (Exception e){
            ...
            if(i == 0){
                ...
                while (iterator.hasNext()){
                    ...
                    while (iteratorValue.hasNext()){
                        Map.Entry entryValue = (Map.Entry) iteratorValue.next();
                        list.add(key+"-"+entryValue.getKey()+"TGI");
                        list.add(key+"-"+entryValue.getKey()+"percent");
                    }
                }
                String[] title = new String[list.size()+1];
                title[0] = "门店名称";
                for (int j=0;j<list.size();j++) {
                    title[j+1] = list.get(j);
                }
